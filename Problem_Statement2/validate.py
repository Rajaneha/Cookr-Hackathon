...
def is_valid(data_frame):
    """Check if the route provided for a vehicle is legal.
    ...
    for _, row in check.iterrows():
        ...
        if time > row.time_window_start+1:
            sys.stderr.write("Error: vehicle " + str(row.vehicle_id)
...
def main(file_name):
    ...
    """
    # Columns are: vehicle_id, order_id, sequence_number
    ...
    valid = data.groupby("vehicle_id").apply(is_valid).tolist()
    valid = [x for sublist in valid for x in sublist]
    total_distance = sum(data.groupby("vehicle_id").apply(route_distance))

    if not sum(valid) == len(data):
        print("Invalid solution")
        exit(1)
    else:
        vans = data.vehicle_id.nunique()
